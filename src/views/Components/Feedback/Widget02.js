...
class Widget02 extends Component {
  render() {
    const { className, cssModule, header, mainText,date, icon, color, footer, link, children, variant, ...attributes } = this.props;
    ...
    return (
      <Card>
        <CardBody className={card.classes} {...attributes}>
          ...
          <div className="text-muted text-uppercase font-weight-bold font-xs">{date}</div>
        </CardBody>
      </Card>
    );
  }
